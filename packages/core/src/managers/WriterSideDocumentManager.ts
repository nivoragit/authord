...
export default class WriterSideDocumentManager extends AbstractDocumentationManager implements DocumentationManager {
    ...
    private async readIhpFile(): Promise<any> {
        const fileExists = await this.fileService.fileExists(this.configPath);
        if (!fileExists) {
            const defaultIhp = `<?xml version="1.0" encoding="UTF-8"?>
<ihp version="2.0">
  <topics dir="topics"/>
</ihp>`;
            await this.fileService.writeNewFile(this.configPath, defaultIhp);
        }
        const raw = await this.fileService.readFileAsString(this.configPath);
        return FileService.parseXmlString(raw);
    }

    private async writeIhpFile(): Promise<void> {
        await this.fileService.updateXmlFile(this.configPath, () => this.ihpData);
    }
    ...
    public async loadAllInstances(): Promise<InstanceProfile[]> {
        ...
        const instanceProfiles = await Promise.all(
            instances.map(async (inst: any) => {
                ...
                if (!(await this.fileService.fileExists(treeFile))) {
                    return null;
                }
    ...
    private async parseInstanceProfile(treeFile: string): Promise<WriterSideInstanceProfile | null> {
        const raw = await this.fileService.readFileAsString(treeFile);
    ...
    public async saveInstance(doc: WriterSideInstanceProfile): Promise<void> {
        ...
        const xmlContent = await this.convertToXmlString(profileObj);
        ...
        await this.fileService.writeNewFile(filePath, fullContent);
    }

    private async convertToXmlString(profileObj: any): Promise<string> {
        const builder = new XMLBuilder({
            ignoreAttributes: false,
            format: true,
            indentBy: await this.fileService.getIndentationSetting(),
    ...
    async removeInstance(docId: string, allTopics: string[]): Promise<boolean> { // todo confirm `const allTopics = TopicsService.getAllTopicsFromTocElement(doc['toc-elements']);` in usages
        const ihp = this.ihpData?.ihp;
        if (!ihp.instance) {
            return false;
        }
        ...
        if (idx > -1) {
            ...
            if (doc) {
                ...
                await Promise.all(
                    allTopics.map(async (tFile: string) => {
                        ...
                        await this.fileService.deleteFileIfExists(p);
                    })
                );
            }
            arr.splice(idx, 1);
            ...
            await this.fileService.deleteFileIfExists(treeFilePath);
