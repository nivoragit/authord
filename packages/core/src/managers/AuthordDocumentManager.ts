...
export default class AuthordDocumentManager extends AbstractDocumentationManager implements DocumentationManager {
    ...
    async initializeConfigurationFile(): Promise<void> {
        ...
        await this.fileService.writeNewFile(this.configPath, '{}');
    ...
    private async parseConfigFile(): Promise<AuthordConfig | undefined> {
        ...
        return this.fileService.readJsonFile(this.configPath);
    }
    ...
    public async saveConfigurationFile(): Promise<void> {
        ...
        await this.fileService.updateJsonFile(this.configPath, () => this.configData!);
    }
    ...
    async removeInstance(docId: string, allTopics: string[]): Promise<boolean> { // todo change parameters in  DocumentationService.deleteDoc// TopicsService.getAllTopicsFromTocElement(foundDoc['toc-elements']);
        ...
        await Promise.all(
            allTopics.map(async (topicFileName: string) => {
                await this.fileService.deleteFileIfExists(path.join(topicsDir, topicFileName));
            })
        );
