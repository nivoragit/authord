...
describe('AuthordDocumentManager', () => {
  ...
  describe('reloadConfiguration', () => {
    it('should load config and update TOC titles', async () => {
      ...
      (AbstractFileService.fileExists as jest.Mock).mockResolvedValue(true);
      (AbstractFileService.readJsonFile as jest.Mock).mockResolvedValue(mockConfig);
    ...
    it('should handle missing config file', async () => {
      (AbstractFileService.fileExists as jest.Mock).mockResolvedValue(false);
  ...
  describe('initializeConfigurationFile', () => {
    it('should create default config file and save it', async () => {
      ...
      expect(AbstractFileService.writeNewFile).toHaveBeenCalledWith(mockConfigPath, '{}');
      expect(manager.configData).toEqual(AuthordDocumentManager.defaultConfigJson());
      expect(AbstractFileService.updateJsonFile).toHaveBeenCalled();
    });
  });
  ...
  describe('saveConfigurationFile', () => {
    it('should save config data if configData is defined', async () => {
      ...
      expect(AbstractFileService.updateJsonFile).toHaveBeenCalledWith(mockConfigPath, expect.any(Function));
    });

    it('should not save if configData is undefined', async () => {
      await manager.saveConfigurationFile();
      expect(AbstractFileService.updateJsonFile).not.toHaveBeenCalled();
    });
  });
  ...
  describe('createDocumentation', () => {
    ...
    it('should handle the case where firstTopic exists', async () => {
      ...
      expect(AbstractFileService.fileExists).toHaveBeenCalled();
      expect(AbstractFileService.updateJsonFile).toHaveBeenCalled();
    });
  });
  ...
  describe('removeDocumentation', () => {
    ...
    it('should remove documentation and delete files', async () => {
      ...
      expect(AbstractFileService.deleteFileIfExists).toHaveBeenCalledWith('/mock/dir/topics/topic1.md');
      expect(AbstractFileService.updateJsonFile).toHaveBeenCalled();
    });
    ...
    it('should return false if documentation is not found', async () => {
      ...
      expect(AbstractFileService.deleteFileIfExists).not.toHaveBeenCalled();
      expect(AbstractFileService.updateJsonFile).not.toHaveBeenCalled();
    });
