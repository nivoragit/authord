...
import { SinglePageComposer } from "@authord/render-core/core/application/single_page_composer.ts";
...
export class ConfluenceTreePublisher {
  ...
  static build(cfg: ConfluenceCfg, imagesDir: string) {
    ...
    const ports: ConfluenceTreePublisherPorts = {
      fs,
      markdown,
      pageRepo,
      attachRepo,
    ...
    return new ConfluenceTreePublisher(ports, cfg, imagesDir);
  }
  ...
  async execute(opts: ExecuteTreeOptions): Promise<ExecuteTreeResult> {
    ...
    if (unreferenced.size) {
      ...
      const orphansRoot = await this.childResolver.ensureChild(rootPageId, orphansTitle);
